# Agent Framework Toolkit @ Anthropic

> This package is aimed at Anthropic as an LLM Provider. Check out the [General README.md](https://github.com/rwjdk/AgentFrameworkToolkit/blob/main/README.md) for other providers and shared features in Agent Framework Toolkit.
...
1. Install the 'AgentFrameworkToolkit.Anthropic' NuGet Package (`dotnet add package AgentFrameworkToolkit.Anthropic`)
2. Get your [Anthropic API Key](https://platform.claude.com/settings/keys)
3. Create an `AnthropicAgentFactory` instance (Namespace: AgentFrameworkToolkit.Anthropic)
4. Use instance to create your `AnthropicAgent` (which is a regular Microsoft Agent Framework `AIAgent` behind the scenes)
