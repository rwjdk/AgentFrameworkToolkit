...
1. Install the 'AgentFrameworkToolkit.XAI' NuGet Package (`dotnet add package AgentFrameworkToolkit.XAI`)
2. Get your [XAI API Key](https://console.x.ai/)
3. Create an `XAIAgentFactory` instance (Namespace: AgentFrameworkToolkit.XAI)
4. Use instance to create your `XAIAgent` (which is a regular Microsoft Agent Framework `AIAgent` behind the scenes)
...
//Create your Agent
XAIAgent agent = agentFactory.CreateAgent(new AgentOptions //Use AgentOptions overload to access more options
{
...
XAIAgentFactory agentFactory = new XAIAgentFactory(new XAIConnection
{
    //Endpoint = "<endpoint>", //Optional if not using default XAI Endpoint
    ApiKey = "<apiKey>",
